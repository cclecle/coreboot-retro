...
unsigned long write_pirq_routing_table(unsigned long addr)
{
	...
	write_pirq_info(pirq_info, 0, PCI_DEVFN(0x0a, 0),
			0x60, 0x1eb8, 0x61, 0x1eb8, 0x62, 0x1eb8, 0x63, 0x1eb8, 1, 0);
	pirq_info++;
	slot_num++;

	/* pci slot 2 */
	write_pirq_info(pirq_info, 0, PCI_DEVFN(0x0c, 0),
			0x61, 0x1eb8, 0x62, 0x1eb8, 0x63, 0x1eb8, 0x60, 0x1eb8, 2, 0);
	pirq_info++;
	slot_num++;

	/* pci slot 3 */
	write_pirq_info(pirq_info, 0, PCI_DEVFN(0x0e, 0),
			0x62, 0x1eb8, 0x63, 0x1eb8, 0x60, 0x1eb8, 0x61, 0x1eb8, 3, 0);
	pirq_info++;
	slot_num++;

	/* 82371AB/EB/MB PIIX4/E/M EIDE Controller */
	write_pirq_info(pirq_info, 0, PCI_DEVFN(0x02, 1),
			0x60, 0x1eb8, 0x61, 0x1eb8, 0x62, 0x1eb8, 0x63, 0x1eb8, 0, 0);
	pirq_info++;
	slot_num++;
